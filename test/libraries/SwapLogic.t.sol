...
contract SwapLogicWrapper {
    ...
    function swap(bytes memory path, uint256 amountIn, uint256 minAmountOut, address recipient)
        public
        returns (uint256 amountOut)
    {
        return SwapLogic.swap(_swapLogicStorage, path, amountIn, minAmountOut, recipient);
    }
...
contract SwapLogicSwapTest is Test {
    ...
    function setUp() public {
        vm.createSelectFork(vm.envString("ETH_RPC_URL"), 22790000); // mainnet fork
        wrapper = new SwapLogicWrapper();
    }
    ...
    function testSwapSXTtoUSDT() public {
        ...
        uint256 amountOut = wrapper.swap(path, amountIn, minAmountOut, recipient);
